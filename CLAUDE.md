...
- **採点 `TodayGrade`**: `gradeSet(d)`＝`printedOn` のある項目を印刷日→紙の順（`printedAs`、印刷時に付く）。「紙を撮る」で `readSelfMarks()` が紙の「自分の判定」の印を AI で読み、`applySelfMarks()` で子ども＝親の初期値として入れる（× は前回の誤答の種類、用語の「説明もできた」は「解けた」に）。親は違う行だけ直す。手で押す場合は行ごとに「子ども」→「親」（子どもを押すと親にも入る）。「できなかった」は誤答の種類が必須（前回の種類が初期値）。「まとめて確定」で `judgeAll()`。「やっていない」は `printedOn` を外して次の印刷に戻す。用語は「解けた」で進み、それ以外は「説明もできた」でないと7日止まり（`applyJudgment` のまま）。
