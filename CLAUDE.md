...
- 類題 `genForItem()`: 元の問題があれば印つきの画像を渡す（`genContent`）。問題数は `problemCount(i)`: 習得中（間隔1日・3日）は3問、保持の確認（7日以上）は1問、暗記もの（`needsWhy` が false）は常に1問。落として1日に戻れば3問に戻る。用語は「〜を書きなさい」の記述形式、説明を求める項目には「説明させる問い」を1問。判定は「出した問題がすべて正解」で「解けた」。今日の分は問題数が合わなければ作り直す。読み間違いはカードの「別の問題」で作り直す。
