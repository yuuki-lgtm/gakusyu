...
### 今日 `TodayTab`（採点／印刷）
- **印刷 `TodayMake`**: `printSet(d)`＝明日までに期日が来る未印刷の項目（候補）。優先順は「定期テストの範囲内（`examRangeIds`）→ 落とした回数 → 期日が古い」。`pickDaily(d)` が上限 `dailyMax()`（localStorage `daily_max`、既定6、設定タブで変更）までを「今日の分」に、残りを繰り越しにする。画面は候補の一覧にチェック（上位が既定で入る）で、親が外す・足すをして「この内容で作る」。外した項目は期日が過ぎたまま残り翌日の候補になる。ヘッダーと夜の作業の行に「今日 6件（繰り越し 12件）」。一度に多数を登録したとき（今日の登録が上限の2倍以上）は `spreadDue()` が初回の期日を翌日から上限ずつ後ろにずらす（タップ登録と答案の読み取りに適用）。上限から溢れた繰り越しは、週次テストの作問（`genPaper` kind 週次）がその教科の分を1問ずつ混ぜ（問題数は +件数、上限30）、問題に `itemId` を付ける。答案を撮ると `judgeCarried()` が○を「解けた」、×・空欄を「できなかった」として元の項目を判定し、新しい項目は作らない。`genForItem()` で類題を自動生成（今日作った類題があれば再利用）し、`genToPaper()` で1つの用紙にして `exportPDF()`。成功した項目に `printedOn` と紙の中の順番 `printedAs` を付ける。生成に失敗した項目のうち、元の問題（`src` の座標）があるものは `printedOrig:true` にして、`genToPaper()` が元の問題の切り出し（`refs` の `crop`、`resolveRefs()` が `cropAround()` で切る）を資料に載せ「元の問題（図N）をもう一度解き、なぜそうなるかを説明する」を1問置く。元の問題が無い項目は次回に回る。「採点待ちの分をもう一度PDFにする」は再生成なしで同じ内容を出す。
