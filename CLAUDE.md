...
### ホーム `HomeTab`
`nextActions(d)` の先頭1つを大きく、残りを「そのあと」に。順序: 単元未登録 → 進度未設定 → **夜の作業（続きがあれば続きの表示）** → 未採点の答案 → ワークの×を登録 → **昨日の分を採点（`printedOn < 今日` の項目）→ 模試を作る（`mockDue`: 定期テストの14・7・3日前の回ごとに範囲の全教科を、1日1〜2教科ずつ。作った用紙の `round` で進み具合を見る）→ 今日の分を印刷（`printSet`）** → 最初の項目 → 週末の進度更新と作問 → **累積テストを作る（前回の累積テスト `lastCumOn` から30日以上。`go("week","cum")` で作る画面を累積で開く）** → 定期テスト14日前 → 3回落ちた項目の診断 → **バックアップを書き出す（`backupOn` から30日以上。書き出すと `backupOn` を更新、同期では新しい日を採用）**。

### 夜の作業 `NightFlow`（タブには出さない。ホームの「いまやること」から `go("night")`）
採点 `TodayGrade` → ×登録 `ItemReg` → 最後のページ `LastPageStep`（教科・教材ごとに最後のページ番号を入れて「ここまで」＝`markDoneUpTo`）→ 明日の分 `TodayMake` の4画面を「次へ」で進む。進み具合は localStorage の `night_step`（今日の日付つき）に保存し、途中で閉じても今日の分は続きから。「終わる」で消す。ホームの「いまやること」は初期設定（単元・進度）の次に「夜の作業」を出す。
