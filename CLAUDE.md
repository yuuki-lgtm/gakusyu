...
## 画面
下のバーは使う頻度で3つ（2026-09-10 に8タブから変更）: **ホーム**（毎日。夜の作業・朝の印刷・今日）／**週末**（週1。`WeekTab`: 作る（手入力は下に畳む）・定期・模試・読解・記述）／**その他**（たまに。`MoreTab` のメニュー: 登録・分析・依頼文・設定）。内側の画面（today／night／reg／ana／exam／export／set）は `go(tab, mode)` で直接開け、バーの選択は所属するグループ（`group`）で光る。`App` が `tab` と `mode`（サブ画面）を持つ。定期は `go("exam")` で週末の「定期」として開く。ホームの「いまやること」は `nextActions(d)` が決める。
